...
class FeatureTrackerDrawer {
   ...
   public:
    void trackFeaturePathRos(const depthai_ros_msgs::TrackedFeaturesConstPtr& features)
    {
...
int main(int argc , char**argv) {
    using namespace std;
    ros::init("tracker_displayer",argc,argv);
    ros::NodeHandle nh("~") ; 
    std::vector<ros::Subscriber> cam_sub_list ; 
    std::vector<std::string> cam_sub_topic_list ;
    std::vector<std::string> track_sub_topic_list ;
    std::vector<FeatureTrackerDrawer> feature_drawer_list ;  
    int max_n = 2 ; 
    if(!nh.getParam("max_n",max_n)) { ROS_INFO("No max_n param detected. Defaulting to %d",max_n)} ;
    for(int i =0 ; i< max_n ; i++)
    {
        std::string sub_i = "/tracker/image"+std::to_string(i) ; 
        std::string track_i = "/tracker/points"+std::to_string(i) ; 
        if(!nh.getParam("cam" + std::to_string(i),sub_i)) { ROS_INFO("No cam%d param detected. Defaulting to %s",i , sub_i.c_str())} ;
        if(!nh.getParam("tracker" + std::to_string(i),track_i)) { ROS_INFO("No tracker%d param detected. Defaulting to %s",i , track_i.c_str())} ;
        cam_sub_topic_list.push_back(sub_i) ;
        track_sub_topic_list.push_back(track_i) ;
        feature_drawer_list.emplace_back("Feature tracking duration (frames)","Window" + std::to_string(i)) ; 
        cam_sub_list.push_back(nh.subscribe(sub_i,2,[&](const sensor_msgs::ImageConstPtr& msg){
        }))  
    }
